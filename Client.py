...
class Client ():
    ...
    def Main_Page(self):
        id_to_task = int(input('하고 싶은 작업을 선택하여 숫자로 입력해 주세요 : 0-종료, 1-사역 확인, 2-사역 면제 쿠폰 사용\n'))
        while (id_to_task!=0 and id_to_task!=1 and id_to_task!=2):
            id_to_task = int(input('입력 에러! 숫자를 1 또는 2로 입력해 주세요 : 0-종료, 1-사역 확인, 2-사역 면제 쿠폰 사용\n'))
        if id_to_task ==1 :
            self.Confirm()
        elif id_to_task==2 :
            self.Request()
    ...
    def Confirm(self):
        ...
        print(pd.read_excel('WorkCycle.xlsx',sheet_name=str(self.ws_names_to_show[id_to_search-1])))
        self.Main_Page()
